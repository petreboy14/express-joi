...
exports.joiValidate = function joiValidate(validations, options) {
  ...
  */
  function validate(req, res, next) {
    ...
    var paramExtras = copyObject(params, items, validations, options.strict);
    var queryExtras = copyObject(query, items, validations, options.strict);
    var bodyExtras = {};

    // Only store body methods on calls that may have a body
    if (method !== "GET" && method !== "DELETE") {
      bodyExtras = copyObject(body, items, validations,options.strict);
    }
 ...
 * @private
 */
function copyObject(from, to, validations, strict) {
  var extras = {};
  if (from) {
    for (var key in from) {
      if (from.hasOwnProperty(key) && (!validations || strict || validations.hasOwnProperty(key))) {
        to[key] = decodeURIComponent(from[key]);
      } else {
        extras[key] = decodeURIComponent(from[key]);
      }
    }
  }
